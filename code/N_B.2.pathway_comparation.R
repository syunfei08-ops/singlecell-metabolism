...
setwd("/xtdisk/xiaojf_group/shangyf/a_result/3.metabolism/2-3.normal/comparation")
...
for (t in tum) {
  ...
  for (cho1 in pw) {
    for (cho2 in pw) {
	  ...
	  tem <- n_scFEA[which(n_scFEA$sample %in% sCho),]
  ...
  data <- corMatr
  pdf(paste0(t,"_corrplot_all.pdf"),width=10,height=10)
...
for (t in tum) {
  n=0
  sCho <- meta_data[which(meta_data[,"tumor"] %in% t),1]
  ...
  pdf(paste0(t,"_groupSixErrorbar.pdf"),width=13,height=11)
  print(p)
  dev.off()
}
